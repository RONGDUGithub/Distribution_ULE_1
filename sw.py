...
def sw(ori_samples, l, h, eps, randomized_bins=1024, domain_bins=1024):
    ...
    location = get_positions(-w / 2, 1 + w / 2, noisy_samples)
    # return EM(n, ns_hist, transform, max_iteration, loglikelihood_threshold) * len(ori_samples)
    return location, transform, EMS(n, ns_hist, transform, max_iteration, loglikelihood_threshold) * len(ori_samples)
...
# min_data = 0
# max_data = 1
# location_sw, transform_sw, distribution_sw = sw(np.array(data), min_data, max_data, epsilon)
