...
def run_experiment(file_name):
    ...
    mechanisms_list = [
        ['duchi', 'piecewise', 'sw', 'duchi', 'piecewise', 'sw'],
        ['duchi', 'piecewise', 'sw', 'duchi', 'piecewise', 'sw', 'duchi', 'piecewise', 'sw'],
        ['duchi', 'piecewise', 'sw', 'duchi', 'piecewise', 'sw', 'duchi', 'piecewise', 'sw', 'duchi', 'piecewise',
         'sw'],
        ['duchi', 'piecewise', 'sw', 'duchi', 'piecewise', 'sw', 'duchi', 'piecewise', 'sw', 'duchi', 'piecewise', 'sw',
         'duchi', 'piecewise', 'sw']
    ]
    epsilons_list = [
    ...
    for mechanisms, epsilons in zip(mechanisms_list, epsilons_list):
        js_sum = 0
        time_sum = 0
    ...
    JS_matrix = np.array(JS_matrix, dtype=object)
    time_matrix = np.array(time_matrix, dtype=object)
    base_name = os.path.splitext(file_name)[0]

    output_filename = './group4/Group4_{}_B.txt'.format(base_name)
    with open(output_filename, 'w') as f:
        f.write('Mechanisms:\n')
        f.write(str(mechanisms_list) + '\n')
        f.write('Epsilons:\n')
...
# 或者处理多个文件
file_names = ['Beta25.xlsx', 'Betawave.xlsx', 'taxi.xlsx', 'Retirement.xlsx']
